    ...
    fetch(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${terminoBusqueda}&format=json&origin=*`)
        ...
        .then(data => {
            ...
            resultadoBusqueda.forEach(result => {
                ...
                resultadoElementos.innerHTML = `<h3>${result.title}</h3><p>${result.snippet}</p><a href="https://en.wikipedia.org/wiki/${result.title}" target="_blank">Leer más</a>`; //POL: Creamos la estructura del DIV con los datos.
                resultadoDiv.appendChild(resultadoElementos); //POL: Introducimos la estructura creada anteriormente en el DIV que queremos.
            });
